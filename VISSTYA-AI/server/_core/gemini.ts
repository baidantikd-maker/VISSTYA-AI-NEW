...
export const GEMINI_MODEL = ENV.geminiModel || "gemini-flash-latest";
...
const parseAnalysis = (text: string): GeminiVisionAnalysis => {
  ...
  const asNullableString = (value: unknown): string | null => {
    ...
    if (
      /^(none|n\/a|not applicable|not visible|not available|unknown|unavailable|indeterminate|indeterminable)/i.test(
        textValue
      )
    ) {
      return null;
...
export async function analyzeMediaWithGemini(
  ...
  claimParts.push(
    `Claimed location: ${claimLocation?.trim() || "not provided"}`
  );
