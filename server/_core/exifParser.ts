...
function findPythonCommand(): string {
  ...
  throw new Error(
    "Python executable not found. Set PYTHON or install Python on the PATH."
  );
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", code => {
      ...
      try {
        ...
        reject(
          new Error(
            `Failed to parse EXIF parser output: ${error instanceof Error ? error.message : String(error)}\nOutput: ${stdout}\nError: ${stderr}`
          )
        );
      }
    });
